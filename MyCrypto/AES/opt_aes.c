...
#define gFun(w) ((S[(w>>16)&0xff]<<24) + (S[(w>>8)&0xff]<<16) + (S[w&0xff]<<8) + (S[(w>>24)&0xff]))

uint32_t rcon[10] = {0x01000000ul, 0x02000000ul, 0x04000000ul, 0x08000000ul,
...
void encrypt(uint8_t plain[], uint8_t key[])
{
    ...
    for(int i = 0; i < 4; ++i){
		s[4*i] = plain[4*i] ^ ((roundKey[i]>>24)&0xff);
		s[4*i+1] = plain[4*i+1] ^ ((roundKey[i]>>16)&0xff);
		s[4*i+2] = plain[4*i+2] ^ ((roundKey[i]>>8)&0xff);
		s[4*i+3] = plain[4*i+3] ^ ((roundKey[i])&0xff);
	}
	for(int i = 1; i < 10; ++i){
	...
	for(int i = 0; i < 4; ++i){
		cypher[4*i] = S[s[4*i]] ^ ((roundKey[40+i]>>24)&0xff);
		cypher[4*i+1] = S[s[(4*(i+1)+1)%16]] ^ ((roundKey[40+i] >> 16) & 0xff);
		cypher[4*i+2] = S[s[(4*(i+2)+2)%16]] ^ ((roundKey[40+i] >> 8) & 0xff);
		cypher[4*i+3] = S[s[(4*(i+3)+3)%16]] ^ ((roundKey[40+i]) & 0xff);
	}
	for(int i = 0; i < 16; ++i){
		printf("%02x", cypher[i]);
	}printf("\n");
	finish = clock();
	printf("encrypt time: %f seconds\n", (double)(finish-start)/CLOCKS_PER_SEC);
	return;
}
void decrypt(uint8_t cypher[], uint8_t key[])
{
	clock_t start, finish;
	start = clock();
	uint8_t s[16], plain[16];
    uint32_t r[4];
	uint32_t roundKey[44];
	getRoundKey(roundKey, key);
	for(int i = 4; i < 40; ++i){
        roundKey[i] = Td0[S[(roundKey[i]>>24)&0xff]] ^ Td1[S[(roundKey[i]>>16)&0xff]] ^ Td2[S[(roundKey[i]>>8)&0xff]] ^ Td3[S[(roundKey[i])&0xff]];
	}
	for(int i = 0; i < 4; ++i){
		s[4*i] = cypher[4*i] ^ ((roundKey[40+i]>>24)&0xff);
		s[4*i+1] = cypher[4*i+1] ^ ((roundKey[40+i]>>16)&0xff);
		s[4*i+2] = cypher[4*i+2] ^ ((roundKey[40+i]>>8)&0xff);
		s[4*i+3] = cypher[4*i+3] ^ ((roundKey[40+i])&0xff);
	}
    for(int i = 9; i > 0; --i){
		r[0] = Td0[s[0]] ^ Td1[s[13]] ^ Td2[s[10]] ^ Td3[s[7]] ^ roundKey[4*i];
		r[1] = Td0[s[4]] ^ Td1[s[1]] ^ Td2[s[14]] ^ Td3[s[11]] ^ roundKey[4*i+1];
	...
	for(int i = 0; i < 4; ++i){
		plain[4*i] = inv_S[s[4*i]] ^ ((roundKey[i]>>24)&0xff);
		plain[4*i+1] = inv_S[s[(4*(i+3)+1)%16]] ^ ((roundKey[i] >> 16) & 0xff);
		plain[4*i+2] = inv_S[s[(4*(i+2)+2)%16]] ^ ((roundKey[i] >> 8) & 0xff);
	    plain[4*i+3] = inv_S[s[(4*(i+1)+3)%16]] ^ ((roundKey[i]) & 0xff);
	}
	for(int i = 0; i < 16; ++i){
		printf("%02x", plain[i]);
	}
	printf("\n");
	finish = clock();
	printf("decrypt time: %f seconds\n", (double)(finish-start)/CLOCKS_PER_SEC);
	return;
}
int main(int argv, char *argc[]) {
  uint8_t key[16] = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};
  uint8_t plain[16] = {0x32, 0x43, 0xf6, 0xa8, 0x88, 0x5a, 0x30, 0x8d, 0x31, 0x31, 0x98, 0xa2, 0xe0, 0x37, 0x07, 0x34};
  uint8_t cypher[16] = {0x39, 0x25, 0x84, 0x1D, 0x02, 0xDC, 0x09, 0xFB, 0xDC, 0x11, 0x85, 0x97, 0x19, 0x6A, 0x0B, 0x32};
  encrypt(plain,key);
  decrypt(cypher, key);
  return 0;
}
